...
run_dataset() {
  ...
  "$PYTHON_BIN" dataset_builder.py     --config "$config"     --output-dir "$dataset_dir"     2>&1 | tee "$LOG_DIR/${name}_dataset_${ts}.log"
}
...
run_training() {
  ...
  "$PYTHON_BIN" train_linear_baseline.py     --config "$config"     --dataset-dir "$dataset_dir"     --output-dir "$training_dir"     2>&1 | tee "$LOG_DIR/${name}_training_${ts}.log"
}
